  ...
  public static class Config {
    ...
    public static Config parse(@NonNull URL mappingFile) {
      try {
        ...
        throw new RuntimeException("Initialization of %s failed!".formatted(I18N.class), ex);
      }
    }
    ...
    private Set localeSet(Locale locale) {
      ...
      try {
        Locale lookup =
            Locale.lookup(Locale.LanguageRange.parse(locale.toLanguageTag()), providedLocales());
        if (lookup == null) {
        ...
        if (set.isEmpty()) {
          try {
            ...
            LOGGER.warn(String.format("Failed to load set file for locale %s", lookup), ex);
          }
          set.ifPresentOrElse(
              i18NSet -> sets.put(locale, i18NSet), () -> sets.put(locale, getDefaultSet()));
        }
        return set.orElseThrow(
            () -> new IllegalStateException("Experiencing an unexpected condition!"));
      } catch (Exception ex) {
        throw new RuntimeException(String.format("Failed to load I18N set for %s", locale), ex);
      }
    }
    ...
    public List<Locale> providedLocales() {
      return mapping.supportedLocales().keySet().stream().map(Locale::forLanguageTag).toList();
    }
  }
