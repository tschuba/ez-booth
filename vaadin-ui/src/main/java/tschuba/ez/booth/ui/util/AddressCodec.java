...
public class AddressCodec {
    ...
    public static String encode(String address) {
      ...
      if (!validator.find()) {
        throw new IllegalArgumentException(
            "Only gRPC address supported! Given URL is not valid: %s".formatted(address));
      }
  ...
  public static class IPv4 {
    ...
    public static byte[] encode(byte[] addressBytes) {
      ...
      if (firstOctet == 172) {
        buffer = ByteBuffer.allocate(3).put(addressBytes, 1, 3); // Store last 3 bytes (24 bits)
        ...
        if (thirdOctet == 178) {
          ...
          buffer = ByteBuffer.allocate(2).put(addressBytes, 2, 2); // Store last 2 bytes (16 bits)}
        }
      } else {
