...
public class ReportViewHelper {
    ...
    @Autowired
    public ReportViewHelper(@NonNull Environment environment,
                            @NonNull ReportingConfig reportingConfig) {
    ...
    public URI reportUrl(@NonNull URI reportFile) {
        ...
        String relativePath = reportingConfig.targetBasePath().relativize(Path.of(reportFile)).toString().replace("\\", "/");
