...
public class ReportingLocalService implements ReportingService {
  ...
  public @NonNull ServiceModel.VendorReportData createVendorReportData(
    ...
    EntityModel.Vendor vendor =
        vendors
            .findById(EntitiesMapper.objectToEntity(vendorKey))
            .orElseThrow((() -> new RecordNotFoundException("Vendor not found: " + vendorKey)));
    ...
    EntityModel.Booth booth =
        booths
            .findById(EntitiesMapper.objectToEntity(vendorKey.booth()))
            .orElseThrow(
                () -> new RecordNotFoundException("Booth not found: " + vendorKey.booth()));
  ...
  public @NonNull URI generateVendorReport(@NonNull DataModel.Vendor.Key... vendors) {
    ...
    try (FileWriter fileWriter = new FileWriter(reportFilePath.toFile())) {
      LOGGER.debug(
          "Rendering {} vendor report items to {}", vendorReportData.size(), reportFilePath);
