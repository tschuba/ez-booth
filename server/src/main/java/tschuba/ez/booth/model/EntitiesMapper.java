...
public class EntitiesMapper {
  ...
  public static final Mappable<DataModel.Booth.Key, EntityModel.Booth.Key> BOOTH_KEY =
      new Mappable<>() {
        ...
        public @NonNull Function<DataModel.Booth.Key, EntityModel.Booth.Key> objectToEntity() {
          return input -> EntityModel.Booth.Key.builder().boothId(input.boothId()).build();
        }

        @Override
        public @NonNull Function<EntityModel.Booth.Key, DataModel.Booth.Key> entityToObject() {
          return entity -> DataModel.Booth.Key.builder().boothId(entity.getBoothId()).build();
        }
      };
  ...
  public static final Mappable<DataModel.PurchaseItem.Key, EntityModel.PurchaseItem.Key>
      PURCHASE_ITEM_KEY =
          new Mappable<>() {
            @Override
            public @NonNull Function<DataModel.PurchaseItem.Key, EntityModel.PurchaseItem.Key>
                objectToEntity() {
              return input -> {
            ...
            @Override
            public @NonNull Function<EntityModel.PurchaseItem.Key, DataModel.PurchaseItem.Key>
                entityToObject() {
              return entity -> {
      ...
      new Mappable<>() {
        ...
        public @NonNull Function<DataModel.PurchaseItem, EntityModel.PurchaseItem>
          ...
          return input -> {
            ...
            return builder.price(input.price()).purchasedOn(input.purchasedOn()).build();
          };
        }
        ...
        public @NonNull Function<EntityModel.PurchaseItem, DataModel.PurchaseItem>
          ...
          return entity -> {
            ...
            return builder.price(entity.getPrice()).purchasedOn(entity.getPurchasedOn()).build();
          };
        }
      };
          ...
          return input -> {
            ...
            if (input.items() != null) {
              List<EntityModel.PurchaseItem> items =
                  input.items().stream().map(PURCHASE_ITEM::objectToEntity).toList();
              builder.items(items);
            }
            return builder.value(input.value()).purchasedOn(input.purchasedOn()).build();
          };
        }
          ...
          return entity -> {
            ...
            if (entity.getItems() != null) {
              List<DataModel.PurchaseItem> items =
                  entity.getItems().stream().map(PURCHASE_ITEM::entityToObject).toList();
              builder.items(items);
            }
            return builder.value(entity.getValue()).purchasedOn(entity.getPurchasedOn()).build();
          };
        }
      };
